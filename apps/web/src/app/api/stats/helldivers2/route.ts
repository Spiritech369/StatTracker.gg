...
async function fetchHelldivers2Stats(): Promise<StatsResponse> {
  ...
  const entities: GameEntity[] = HD2_WEAPONS_DATA.map((weapon, i) => {
    ...
    return {
      ...
      trend,
      trendChange,
      difficulty: seededRandom(seed + 400) > 0.6 ? 'Hard' : seededRandom(seed + 400) > 0.25 ? 'Medium' : 'Easy',
  ...
  const bestPrimary = entities.filter(e => e.role === 'Primary').sort((a, b) => b.winrate - a.winrate)[0]
  const bestSupport = entities.filter(e => e.role === 'Support').sort((a, b) => b.winrate - a.winrate)[0]
  const bestSecondary = entities.filter(e => e.role === 'Secondary').sort((a, b) => b.winrate - a.winrate)[0]
    ...
    metaInsights: [
      {
        ...
        items: [
          ...
          { label: 'Avg Mission Success', value: `${avgWR}%`, sublabel: `${entities.length} weapons tracked` },
      ...
      },
      {
        title: 'Loadout Picks',
        items: [
          { label: 'Best Primary', value: bestPrimary?.name || 'N/A', sublabel: `${bestPrimary?.winrate || 0}% mission success` },
          { label: 'Best Support', value: bestSupport?.name || 'N/A', sublabel: `${bestSupport?.winrate || 0}% mission success` },
          { label: 'Best Secondary', value: bestSecondary?.name || 'N/A', sublabel: `${bestSecondary?.winrate || 0}% mission success` },
          { label: 'Stratagem Synergy', value: `${(seededRandom(dayFactor * 7) * 30 + 60).toFixed(0)}%`, sublabel: 'Avg team coordination' },
        ],
      },
      {
        title: 'Difficulty Rating',
        items: [
          { label: 'Easy Weapons', value: `${entities.filter(e => e.difficulty === 'Easy').length}`, sublabel: 'Beginner friendly' },
          { label: 'Medium Weapons', value: `${entities.filter(e => e.difficulty === 'Medium').length}`, sublabel: 'Moderate skill floor' },
          { label: 'Hard Weapons', value: `${entities.filter(e => e.difficulty === 'Hard').length}`, sublabel: 'High skill ceiling' },
          { label: 'Helldive Ready', value: `${entities.filter(e => e.winrate >= 50).length}/${entities.length}`, sublabel: 'Weapons ≥ 50% success' },
        ],
      },
    ],
  }
