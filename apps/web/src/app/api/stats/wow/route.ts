...
async function fetchWoWStats(): Promise<StatsResponse> {
  ...
  const bestHealer = entities.filter(e => e.role === 'Healer').sort((a, b) => b.winrate - a.winrate)[0]
    ...
    metaInsights: [
      {
        ...
        items: [
          ...
          { label: 'Avg Win Rate', value: `${avgWR}%`, sublabel: `${entities.length} specs tracked` },
      ...
      },
      {
        title: 'Mythic+',
        items: [
          { label: 'Top Tank', value: bestTank?.name || 'N/A', sublabel: `${bestTank?.winrate || 0}% WR` },
          { label: 'Top Healer', value: bestHealer?.name || 'N/A', sublabel: `${bestHealer?.winrate || 0}% WR` },
          { label: 'Top DPS', value: bestDPS?.name || 'N/A', sublabel: `${bestDPS?.winrate || 0}% WR` },
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'S-Tier Specs', value: `${entities.filter(e => e.tier === 'S').length}`, sublabel: 'Dominant in all content' },
          { label: 'Tank Viability', value: `${entities.filter(e => e.role === 'Tank').length} specs`, sublabel: 'Above 48% win rate' },
          { label: 'Healer Balance', value: `${entities.filter(e => e.role === 'Healer').length} specs`, sublabel: 'In meta window' },
        ],
      },
    ],
  }
