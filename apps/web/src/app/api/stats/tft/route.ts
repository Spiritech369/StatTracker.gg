...
const TFT_COMPS_DATA: {
  ...
  },
  {
    ...
    baseWR: 18.9,
    basePR: 3.1,
    units: ['Rumble', 'Ziggs', 'Fiddlesticks'],
  },
  {
    name: 'Arcanist',
    cost: '4',
...
async function fetchTFTStats(): Promise<StatsResponse> {
  ...
  const bestT4 = entities.reduce((a, b) =>
    (a.placementRate || 0) > (b.placementRate || 0) ? a : b,
  )
  const mostPlayed = entities.reduce((a, b) => (a.pickrate > b.pickrate ? a : b))
  const avgT4 = (
    entities.reduce((a, b) => a + (b.placementRate || 0), 0) / entities.length
  ).toFixed(1)
