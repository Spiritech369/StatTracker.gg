...
const UNIQUE_HEROES = Array.from(
  new Map(DL_HEROES_DATA.map(h => [h.name, h])).values()
)
  ...
  const response: StatsResponse = {
    ...
    metaInsights: [
      ...
      },
      {
        title: 'Lane Meta',
        items: [
          { label: 'Lane 1 (Carry)', value: entities.filter(e => e.role === 'Carry').sort((a, b) => b.winrate - a.winrate)[0]?.name || 'N/A', sublabel: `${entities.filter(e => e.role === 'Carry').sort((a, b) => b.winrate - a.winrate)[0]?.winrate || 0}% WR` },
          { label: 'Lane 2 (Flex)', value: entities.filter(e => e.role === 'Flex').sort((a, b) => b.winrate - a.winrate)[0]?.name || 'N/A', sublabel: `${entities.filter(e => e.role === 'Flex').sort((a, b) => b.winrate - a.winrate)[0]?.winrate || 0}% WR` },
          { label: 'Lane 3 (Tank)', value: entities.filter(e => e.role === 'Tank').sort((a, b) => b.winrate - a.winrate)[0]?.name || 'N/A', sublabel: `${entities.filter(e => e.role === 'Tank').sort((a, b) => b.winrate - a.winrate)[0]?.winrate || 0}% WR` },
          { label: 'Lane 4 (Support)', value: entities.filter(e => e.role === 'Support').sort((a, b) => b.winrate - a.winrate)[0]?.name || 'N/A', sublabel: `${entities.filter(e => e.role === 'Support').sort((a, b) => b.winrate - a.winrate)[0]?.winrate || 0}% WR` },
        ],
      },
    ],
  }
