...
async function fetch2XKOStats(): Promise<StatsResponse> {
  ...
  const bestPower = entities
    .filter((e) => e.role === 'Power')
    .sort((a, b) => b.winrate - a.winrate)[0]
  const bestSpeed = entities
    .filter((e) => e.role === 'Speed')
    .sort((a, b) => b.winrate - a.winrate)[0]
  const bestTech = entities
    ...
    metaInsights: [
      ...
      },
      {
        ...
        items: [
          {
            label: 'Best Power',
            value: bestPower?.name || 'N/A',
            sublabel: `${bestPower?.winrate || 0}% WR`,
          },
          {
            label: 'Best Speed',
            value: bestSpeed?.name || 'N/A',
            sublabel: `${bestSpeed?.winrate || 0}% WR`,
          },
          {
            label: 'Best Technical',
            value: bestTech?.name || 'N/A',
            sublabel: `${bestTech?.winrate || 0}% WR`,
          },
      ...
      },
      {
        ...
        items: [
          ...
          {
            label: 'Comeback Rate',
            value: `${(seededRandom(dayFactor * 37) * 18 + 22).toFixed(1)}%`,
            sublabel: 'After losing round 1',
          },
          {
            label: 'Character Diversity',
            value: `${(seededRandom(dayFactor * 43) * 6 + 10).toFixed(1)}`,
            sublabel: 'Avg unique per lobby',
          },
        ],
      },
    ],
